...
public class TowerBullet extends Image {
	private static final float SIZE = 32f;
	private static int shootingRange = 15;
	private Rectangle range;
	private int towerDamage = 200;
	private boolean shooting;
	public Creeps ShotCreep=null;
	private ShapeRenderer debugRenderer;
	
	public TowerBullet(float x, float y, Texture tower){
		super(tower);
		...
		int ReloadTime = 200, WaitingFire=0;
		public void physics(){
			if(WaitingFire>= ReloadTime){
				...
				if(!shooting){
					for (int i=0; i< GameScreen.Wave.size(); i++){
						if(Intersector.intersectRectangles(GameScreen.Wave.get(i).Rect, range)){
							...
							GameScreen.createBulletBlack(this.getX(),this.getY(),GameScreen.Wave.get(i));
				...
				if(ShotCreep!=null){
					...
					if(Intersector.intersectRectangles(ShotCreep.Rect,range)){
						...
						GameScreen.createBulletBlack(this.getX(),this.getY(),ShotCreep);
