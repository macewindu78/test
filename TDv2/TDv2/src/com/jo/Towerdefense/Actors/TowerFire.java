...
public class TowerFire extends Image {
	...
	public TowerFire(float x, float y, Texture tower){
		super(tower);
		...
		int ReloadTime = 10, WaitingFire=0;
		public void physics(){
			if(WaitingFire>= ReloadTime){
				...
				if(!shooting){
					for (int i=0; i< GameScreen.Wave.size(); i++){
						if(Intersector.intersectRectangles(GameScreen.Wave.get(i).Rect, range)){
							...
							GameScreen.createBulletFire(this.getX(),this.getY(),GameScreen.Wave.get(i));
				...
				if(ShotCreep!=null){
					...
					if(Intersector.intersectRectangles(ShotCreep.Rect,range)){
						...
						GameScreen.createBulletFire(this.getX(),this.getY(),ShotCreep);
